...
It is possible to create and use all the index types described in xref:4.4@cypher-manual:ROOT:indexes-for-search-performance/index.adoc[Cypher Manual -> Indexes].

This section demonstrates how to work with indexes with an example of a user database.
For information about how to create an index on all `User` nodes that have a `username` property, see xref:4.4@cypher-manual:ROOT:indexes-for-search-performance/index.adoc#administration-indexes-create-a-single-property-index-for-nodes[Cypher Manual -> Create a single-property index for nodes].
