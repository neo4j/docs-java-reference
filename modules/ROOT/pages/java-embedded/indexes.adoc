...
It is possible to create and use all the index types described in link:{neo4j-docs-base-uri}/cypher-manual/4.1/administration/indexes-for-search-performance/[Cypher Manual -> Indexes for search performance].

This section demonstrates how to work with indexes with an example of a user database.
For information about how to create an index on all `User` nodes that have a `username` property, see link:{neo4j-docs-base-uri}/cypher-manual/4.1/administration/indexes-for-search-performance/#administration-indexes-create-a-single-property-index[Cypher Manual -> Create a single-property index].
