...
[NOTE]
====
The memory resource tracking API for procedure framework is available for preview.
...
To do this you need to inject a `org.neo4j.procedure.memory.ProcedureMemory` as a field in your procedure class.
...
* `ProcedureMemoryTracker newTracker()`,
  creates a new memory resource tracker that is bound to the current transaction.
* `HeapEstimator heapEstimator()`,
  used to estimate the heap size of classes and instances.
* `HeapTrackingCollectionFactory collections()`,
  lets you create collections that have build-in memory tracking of their internal structure.

It's usually difficult and time-consuming to implement memory resource tracking.
Here's a few considerations and caveats that are worth keeping in mind:
...
- It's common to not know the size of an instance before it has been allocated.
  ...
  The memory tracker implementation try to guard against this
...
- It's cumbersome in Java to know when an instance has been garbage collected.
  Typically, you will register the release of memory at the point when it's possible for that memory to be garbage collected.
  To account for this, memory trackers might internally choose not to register the release of memory instantaneously.
...
public class MyProcedures {
    ...
    public Stream<Output> memoryHungryRange(@Name("size") int size) {
        ...
        final var shallowSizeOfLong = memory.heapEstimator().shallowSizeOfInstance(long.class);

        // Register the allocation of the long array below
        tracker.allocateHeap(size * shallowSizeOfLong);
