...
Please note: when you are connecting to a remote process or, in some cases, local that is running as a service please choose a `Remote Process` connection option.
<port> is value that is configured by `com.sun.management.jmxremote.port` property.
