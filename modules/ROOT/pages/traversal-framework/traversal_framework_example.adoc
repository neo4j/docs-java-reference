...
A link:{org-neo4j-graphdb-traversal-TraversalDescription}[`org.neo4j.graphdb.traversal.TraversalDescription`^] is built using a fluent interface.
Such a description can then spawn several link:{org-neo4j-graphdb-traversal-Traverser}[`org.neo4j.graphdb.traversal.Traverser`^] objects.
...
If you are not interested in the Paths, but the Nodes, you can turn the traverser into an iterable of nodes like this:
...
== Example
Below is an example of how to implement a procedure using the Traversal Framework.
This allows the Traversal Framework to be used side by side with Cypher. The transaction and logger are made available through the Procedure Framework:
...
public Stream<TestProcedure.PathResult> findFriends(@Name("person") Node person) {
    ...
    return stream(traverse.iterator()).map(TestProcedure.PathResult::new);
}
