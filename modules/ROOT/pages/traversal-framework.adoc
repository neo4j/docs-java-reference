...
[NOTE]
====
The Uniqueness should always be set manually when using the `BidirectionalTraversalDescription` as the default of `Uniqueness.NODE_GLOBAL`
will result in no paths being returned as the two sides will never be able to collide.
====
...
=== Collision Evaluator
The `CollisionEvaluator` takes a `PathEvaluator`
...
// TODO: Wasn't really sure what these did :P
