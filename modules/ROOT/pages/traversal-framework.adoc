...
A bidirectional traversal consists of two traversals starting from different nodes, which will return the paths where both traversals collide.
...
[NOTE]
====
The Uniqueness is shared with both traversals. There will be no results if the Uniqueness is set to `Uniqueness.NODE_GLOBAL`,
as both traversals need to traverse to a node to get a collision.
As a consequence, always set the uniqueness manually when using the `BidirectionalTraversalDescription` as the default is `Uniqueness.NODE_GLOBAL`.
====
...
// TODO: add example of a custom SideSelector
=== Collision Detector
A `BranchCollisionDetector` returns the paths which can be combined from the given traversals.

=== Collision Policies
A `CollisionPolicy` evaluates whether a path should be included once a collision occurs.
