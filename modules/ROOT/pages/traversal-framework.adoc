...
For a user of the traversal API it is easier to implement the PathExpander/RelationshipExpander interface,
since it only contains one method -- the method for getting the relationships from a path/node,
the methods that the Expander interface adds are just for building new Expanders.
...
TraversalDescription td = transaction.traversalDescription()
    .expand(StandardExpander.create(RelationshipType.withName("A"), Direction.OUTGOING));
return td.traverse(start);
----

The same expander can also be created with the `PathExpanders` static methods.
[source, java, role="nocopy"]
----
TraversalDescription td = transaction.traversalDescription()
...
==== PathExpanderBuilder
It is also possible to define a custom `PathExpander` with the `PathExpanderBuilder`.
...
* `emptyOrderedByType()` -- expands no relationships, guarantees the order of how types will be epxanded when any are added
...
That PathExpander can be further improved by the following methods:
